...
{
    $result = db_fetch_array("SELECT p.* FROM `products` p WHERE p.id_cate = $id");
    return $result;
}

function get_list_products_by_id_orgin($id)
{
    $result = db_fetch_array("SELECT p.* FROM `products` p WHERE p.id_orgin = $id");
    return $result;
}
...
function get_list_products_by_id_orgin_sort($id, $sort)
{
    if ($sort == 0) {
        ...
        $result = db_fetch_array("SELECT * FROM `products` p WHERE p.id_orgin = $id 
            ORDER BY p.price ASC");
    } elseif ($sort == 4) {
        $result = db_fetch_array("SELECT * FROM `products` p WHERE p.id_orgin = $id 
            ORDER BY p.price DESC");
